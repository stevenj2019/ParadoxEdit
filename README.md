...
* Load Order Resolution (removing files based on paradox load order process)
* InlineEditiing for values in file (keys to come later)
* .bak file generation (with safe mode)
* Per-source Searching 
* Modficiation painting (green=added, yellow=modified, red=marked for deletion)

### Advanced/Bulk Operation Functionality
