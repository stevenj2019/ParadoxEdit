...
# from ParadoxParser.ParadoxNodes import GenericLocKey, GenericLegacyLocKey, GenericBlock, GenericKeyValue
...
# #TODO: these have no way to create keys.... yet
# class BaseLocaliseForm(QDialog):
#     def __init__(self, app_controller, name) -> None:
#         super().__init__()
...
#     def _loc_key_widget(self, node:GenericLocKey|GenericLegacyLocKey) -> QTextEdit:
...
#             self.save_file = None
#         self.localisation_fields.append(self._loc_key_widget(loc_node))
#         self._lower_form_body()
#         self.exec_()

# class LocaliseEventForm(BaseLocaliseForm):
#     def __init__(self, app_controller, node) -> None:
...
#         loc_nodes = list()
#         loc_entries = [node for node in self.node.nodes if isinstance(node, GenericKeyValue) and node.key == node_key]
#         for entry in loc_entries:
#             if isinstance(entry, GenericBlock):
#                 text_node = next((node for node in entry.nodes if isinstance(node, GenericKeyValue) and node.key == loc_key), None)
#                 if text_node:
