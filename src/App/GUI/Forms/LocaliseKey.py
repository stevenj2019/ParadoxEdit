...
class BaseLocaliseForm(QDialog):
    def __init__(self, app_controller, name):
        ...
        self.resize(550, 150)#this will likely need an edit
...
# class LocaliseNodeForm(QDialog):
#     def __init__(self, app_controller, key:str=None):
#         super().__init__()
        # self.app_controller = app_controller
        # self.category = app_controller.registry.get_category(CATEGORY)
        # self.category_meta = app_controller.registry.get_category_metadata(CATEGORY)
        # self.loc_key = key
        # if self.loc_key and self.loc_key in self.category_meta.keys():
        #     self.file_selected = self.category_meta[self.loc_key]["file"]
        #     self.node_selected = self.category_meta[self.loc_key]["node"]
        # else:
        ...
        # self.localisation_label = QLabel(self.loc_key)
    ...
    #             self.file_dropdown.setCurrentIndex(index)
    #             self.file_dropdown.setEnabled(False)
    #             self.file_selected = self.category.files[self.file_dropdown.itemText(index)]
    #     if not self.file_selected:
    #         text = self.file_dropdown.currentText()
    #         self.file_selected = self.category.files[text]
    ...
    # def _change_save_file(self, index):
    #     file = self.file_dropdown.itemText(index)
    #     self.file_selected = self.category.files[file]
    ...
    # def _submit(self):
    #     self._handle_localisation_field()
    #     new_value = self.localisation_text.toPlainText().replace("\n", "\\n")
    #     if self.node_selected is not None:
    #         self.app_controller.request_node_mutation.emit(
    #             NodeMutationRequest(file=self.file_selected, node=self.node_selected, node_value=self.node_selected, value=new_value)
    #         )
    #     else:
    #         node = GenericLocKey(self.loc_key, new_value)
    #         self.app_controller.request_block_mutation.emit(
    #             BlockMutationRequest.add(self.file_selected, len(self.file_selected.nodes)+1, node, self.file_selected)
    #         )
