...
class AddNewGFXForm(QDialog):
    def __init__(self, app_controller: AppController, file: FileReference) -> None:
        ...
        for file in self.interface_directory.iter_files():
            self.file_dropdown.addItem(file.file.filename, userData=file)
    ...
    def _select_save_folder(self) -> None:
        path = gfx_save_folder_selector(self, self.source.file_path / "gfx")
        if path:
            self.storage_folder_path = path
            self.storage_folder_path_element_text.setText(str(path))

    def _submit(self) -> None:
        self.file_list = self.file_selector.file_list
        sprites = []

        def image_collection_loop(sprites: list, path: Path) -> list[Path]:
            if path.is_dir():
                for root, dirs, files in os.walk(path):
                    for name in files:
                        if name.endswith("dds") or name.endswith("png"):
                            sprites.append(Path(root) / name)
            else:
        ...
        def generate_icon_files(save_to: Path, sprites: list[Path]) -> list[FileReference]:
            ...
            for sprite in sprites:
                ...
                new_file = FileReference(
                    directory, IconFile.add(new_path, sprite), ParadoxContext, False
                )
                icons.append(new_file)
            return icons

        def generate_blocks(
            get_shines: bool, sprite: FileReference, file_path: Path
        ) -> list[GenericNode]:
            ...
            if get_shines:
                return [
                    GenericComment(f"Focus Icon for: {icon_file.filepath.stem}"),
        ...
        if not self.storage_folder_path or not self.file_list:
            form_missing_value(self)
            return
        for path in self.file_list:
            image_collection_loop(sprites, path)
        icons = generate_icon_files(self.storage_folder_path, sprites)
