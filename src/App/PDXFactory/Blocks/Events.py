...
def country_event_block(event_id:str=None) -> GenericBlock:
...
def news_event_block(event_id:str=None) -> GenericBlock:
    event_id = event_id if event_id else None
...
def event_essentials(event_id:str=None) -> list[GenericBlock]:
    event_id = event_id if event_id else "namespace_here.1"
    return [
        GenericKeyValue("id", 
                        GenericToken(event_id)),
        GenericKeyValue("title", 
                        GenericToken(f"{event_id}.t")),
        GenericKeyValue("description", 
                        GenericToken(f"{event_id}.d")),
        GenericKeyValue("picture", 
                        GenericToken(f"GFX_{event_id}_1")),
        GenericKeyValue("fire_only_once", 
                        GenericBool(False)),
        GenericKeyValue("is_triggered_only", 
                        GenericBool(False)),
    ]
###            ###
#  EVENT BLOCKS  #
