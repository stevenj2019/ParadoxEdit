...
def country_event_block(event_id: str = None) -> GenericBlock:
    ...
    return GenericBlock(
        "country_event", [*event_essentials(event_id), immediate_block(), option_block(event_id)]
    )
...
def news_event_block(event_id: str = None) -> GenericBlock:
    event_id = event_id if event_id else None
    return GenericBlock(
        "news_event", [*event_essentials(event_id), immediate_block(), option_block(event_id)]
    )
...
def option_block(option_name: str = None) -> GenericBlock:
    ...
    return GenericBlock(
        "option",
        [GenericKeyValue("name", GenericToken(option_name)), GenericComment("##YOUR CODE HERE")],
    )
